...
# BG - below from fb cookie example
...
class FacebookClientFlowMixin(object):
    ...
    # BG we call this to get user to grant permission? passing in our callback?
    ...
    # BG exact same as above, but extended permission are required field?  wtf? why bother?
    ...
    def get_authenticated_user(self, callback):
        ...
        access_token = self.request.args.get('tr_fb_access_token')
        expires_seconds = int(self.request.args.get('expires_seconds'))
        now = datetime.datetime.now()
        token_expires = now + datetime.timedelta(seconds=expires_seconds)
        ...
        try:
            ...
            user = { 'name' : profile['name'],
                    ...
                    'profile_image_url':      'https://graph.facebook.com/%s/picture' % profile['id'], #?type=large square small
            ...
            logging.error('failed to retrieve facebook profile in auth', e)
            return callback(None)
    ...
    # BG - called back after get_authenticated_user call, which calls back to application caller
    def _on_get_user_info(self, callback, session, users):
        ...
        return callback({
            ...
            'uid':             user['uid'],
            'locale':          user['locale'],
            'profile_image_url':      user['pic_square'],
